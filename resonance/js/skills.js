  ...
  office: {
    ...
    skills: [
      ...
      { id:'pc_1', name:'ПК ур.1', icon:'🖥',
        desc:'Старый монитор с треснутым экраном. Запустить — значит видеть. Не запустить — значит быть слепым.',
        maxLevel:1, cost:[1], effect:'pc_1',
        row:0, col:1, requires:[] },

      { id:'cleanup', name:'УБОРКА', icon:'🧹',
        desc:'Мусор, окурки, чужие следы. Вычистить всё. Офис должен быть твоим.',
        maxLevel:1, cost:[1], effect:'office_stage_1',
        row:1, col:1, requires:['pc_1','door_1'] },

      { id:'corridor_cleanup', name:'УБОРКА КОРИДОРА', icon:'🪣',
        desc:'Коридор завален хламом со дня одного из инцидентов. Расчистишь — камеры 8А и 8Б покажут больше.',
        maxLevel:1, cost:[1], effect:'corridor_cleanup',
        row:1, col:2, requires:['cleanup'] },

      { id:'corridor_repair', name:'РЕМОНТ КОРИДОРА', icon:'🪟',
        desc:'Трещины от пола до потолка — следы чего-то, что не должно было случиться. Заделать. Камеры светлее.',
        maxLevel:1, cost:[1], effect:'corridor_repair',
        row:2, col:2, requires:['corridor_cleanup'] },

      { id:'office_restore_2', name:'ПОЧИНКА СТЕН', icon:'🔨',
        desc:'Штукатурка, гвозди, банки из-под краски. Офис перестаёт напоминать место преступления.',
      ...
      { id:'office_restore_3', name:'ОБУСТРОЙСТВО', icon:'🛋',
        desc:'Стол, лампа, плакат с кошкой на стене. Становится чуть легче дышать. +1 заряд способностей. Открывает антенны и датчики.',
        maxLevel:1, cost:[1], effect:'office_stage_3',
        row:3, col:1, requires:['office_restore_2','door_2'] },

      { id:'pc_2', name:'ПК ур.2', icon:'🖥',
        desc:'Оперативная память, видеокарта с барахолки. Камеры стали чище — теперь видно чуть больше правды.',
      ...
      { id:'soundproof', name:'ШУМОИЗОЛЯЦИЯ', icon:'🔇',
        desc:'Толстые маты на стенах. Запах не выходит — Ползун учует офис в 3× реже. Дверь закрывается сама, без расхода энергии.',
        maxLevel:1, cost:[1], effect:'soundproof',
        row:5, col:1, requires:['pc_2','door_2'] },

      { id:'pc_3', name:'ПК ур.3', icon:'🖥',
        desc:'Топовая сборка. Камеры в полном цвете, без затемнений, без шума. Ты видишь всё — и не факт что хочешь.',
        maxLevel:1, cost:[1], effect:'pc_3',
        row:6, col:1, requires:['soundproof','door_2'] },

      { id:'solar_panels', name:'СОЛН. ПАНЕЛИ', icon:'☀',
        desc:'Панели на крыше гудят под дождём. +0.20% энергии в секунду. Тень у двери — прогоняется автоматически, без потери рассудка.',
        maxLevel:1, cost:[1], effect:'solar_panels',
        row:7, col:1, requires:['pc_3'] },

      // ═══ ВЕТКА АНТЕННА (col 0) ═══════════════════════
      { id:'antenna_1', name:'АНТЕННА ур.1', icon:'📡',
        desc:'Проволока, банки, изолента. Выглядит смешно. Работает. −50% помех на камерах.',
        maxLevel:1, cost:[1], effect:'antenna_1',
        row:3, col:0, requires:['office_restore_3'] },

      { id:'antenna_2', name:'АНТЕННА ур.2', icon:'📡',
        desc:'Медная решётка с контуром. Шум Тени больше не перекрывает сигнал. −80% помех.',
        maxLevel:1, cost:[1], effect:'antenna_2',
        row:4, col:0, requires:['antenna_1'] },

      { id:'antenna_3', name:'АНТЕННА ур.3', icon:'📡',
        desc:'Военная разработка. Сигнал максимальный. Помехи исчезли. Тишина в эфире — хуже любого шума.',
        maxLevel:1, cost:[1], effect:'antenna_3',
        row:5, col:0, requires:['antenna_2'] },

      // ═══ ВЕТКА ДАТЧИК (col 2) ═════════════════════════
      { id:'motion_1', name:'ДАТЧИК ур.1', icon:'🎯',
        desc:'Дешёвый датчик из хозмага. Пищит когда что-то движется внутри здания.',
        maxLevel:1, cost:[1], effect:'motion_1',
        row:3, col:2, requires:['office_restore_3'] },

      { id:'motion_2', name:'ДАТЧИК ур.2', icon:'🎯',
        desc:'ИК-сенсор с военного склада. Видишь всё — здание и периметр. Теперь ты знаешь когда они приближаются.',
        maxLevel:1, cost:[1], effect:'motion_2',
        row:4, col:2, requires:['motion_1'] },

      { id:'motion_3', name:'ДАТЧИК ур.3', icon:'🎯',
        desc:'12 датчиков по всей территории. NPC виден везде. Незнание больше не убьёт тебя.',
      ...
      { id:'door_1', name:'ДВЕРЬ ур.1', icon:'🚪',
        desc:'Деревяшка с засовом. Хлипкая. NPC ломает за 15 секунд. Лучше чем ничего — но не намного.',
        maxLevel:1, cost:[1], effect:'door_1',
        row:0, col:3, requires:['pc_1'] },

      { id:'o_barricade', name:'БАРРИКАДА', icon:'🪵',
        desc:'Ящики, доски, металлолом у двери. Некрасиво, зато ещё 5 секунд жизни.',
        maxLevel:1, cost:[1], effect:'barricade',
        row:1, col:3, requires:['door_1'] },

      { id:'door_2', name:'ДВЕРЬ ур.2', icon:'🛡',
        desc:'Сталь на заклёпках. NPC ломает вдвое дольше. −20% расхода энергии двери. Чувствуется надёжнее.',
        maxLevel:1, cost:[1], effect:'door_2',
        row:2, col:3, requires:['o_barricade','office_restore_2'] },

      { id:'o_lock', name:'ЗАСОВ', icon:'🔩',
        desc:'Засов из арматуры. Просто. Надёжно. +10 секунд пока они ломятся снаружи.',
        maxLevel:1, cost:[1], effect:'reinforced_lock',
        row:3, col:3, requires:['door_2'] },

      { id:'door_3', name:'ДВЕРЬ ур.3', icon:'🤖',
        desc:'Биометрический замок. Закрывается сама при приближении NPC. Не тратит энергию. Дверь умнее тебя.',
      ...
      { id:'o_reinforce', name:'УКРЕПЛЕНИЕ', icon:'⛓',
        desc:'Стальные пластины поверх стали. Ползун в ярости грызёт вдвое дольше. Слышно даже через шумоизоляцию.',
      ...
      { id:'o_projector', name:'ПРОЖЕКТОР', icon:'💡',
        desc:'Военный прожектор на cam10. Тень прогоняется неограниченно — без потери рассудка. Свет — единственное что её боится.',
  ...
  hunter: {
    ...
    skills: [
      ...
      { id:'h_snare', name:'СИЛОК', icon:'🪢',
        desc:'Верёвочная петля — старый метод. NPC задерживается на 10 сек. Простое решение для простой угрозы.',
        maxLevel:1, cost:[1], effect:'snare',
        row:0, col:0, requires:[] },

      { id:'h_noisemaker', name:'ШУМЕЛКА', icon:'🔔',
        desc:'Консервные банки на верёвке у прохода. Звенит — значит кто-то идёт. Работает пассивно.',
        maxLevel:1, cost:[1], effect:'noisemaker',
        row:0, col:1, requires:[] },

      { id:'h_spike_trap', name:'САМОДЕЛЬНЫЕ ШИПЫ', icon:'📌',
        desc:'Гвозди в доске, вбитой в пол. NPC задерживается на 15 сек и отступает на зону. Грубо — но работает.',
        maxLevel:1, cost:[1], effect:'spike_trap',
        row:1, col:0, requires:['h_snare'] },

      { id:'h_tripwire', name:'РАСТЯЖКА', icon:'🧵',
        desc:'Леска поперёк прохода. Мгновенное оповещение + NPC замедлен на 8 сек. Перекрывает 2 зоны.',
        maxLevel:1, cost:[1], effect:'tripwire',
        row:1, col:1, requires:['h_noisemaker'] },

      { id:'h_bear_trap', name:'КАПКАН', icon:'⚙️',
        desc:'Стальные челюсти у порога. NPC застрял на 20 секунд. Слышно как они воют. Сбрасывается сам.',
        maxLevel:1, cost:[1], effect:'bear_trap',
        row:2, col:0, requires:['h_spike_trap'] },

      { id:'h_smoke_bomb', name:'ДЫМОВАЯ ШАШКА', icon:'💨',
        desc:'Самодельная шашка из удобрений. Ослепляет камеры 9, 10, 11 на 12 сек. 2 раза за смену.',
      ...
      { id:'h_flash_1', name:'ФОНАРЬ ур.1', icon:'🔦',
        desc:'Литий-ионный элемент питания. −20% расхода батареи фонаря. Темнота отступает чуть медленнее.',
        maxLevel:1, cost:[1], effect:'flash_1',
        row:0, col:2, requires:[] },

      { id:'h_flash_2', name:'ФОНАРЬ ур.2', icon:'🔦',
        desc:'Усиленный рефлектор. +20% шанс прогнать тень вспышкой. Свет стал резче.',
        maxLevel:1, cost:[1], effect:'flash_2',
        row:1, col:2, requires:['h_flash_1'] },

      { id:'h_flash_3', name:'ФОНАРЬ ур.3', icon:'🔦',
        desc:'Подключён к сети. Батарея бесконечная — расход не считается. Открывает ПРОЖЕКТОР в офисе.',
      ...
      { id:'h_decoy_1', name:'ПРИМАНКА ур.1', icon:'📦',
        desc:'Мясо в жестяной банке. Ползун отвлекается — забывает про тебя. 1 заряд на камеру 2.',
        maxLevel:1, cost:[1], effect:'decoy_1',
        row:0, col:3, requires:[] },

      { id:'h_decoy_2', name:'ПРИМАНКА ур.2', icon:'📦',
        desc:'Кровь и кости. +1 заряд на камеру 3. Ползун не думает — он ест.',
        maxLevel:1, cost:[1], effect:'decoy_2',
        row:1, col:3, requires:['h_decoy_1'] },

      { id:'h_decoy_3', name:'ПРИМАНКА ур.3', icon:'📦',
        desc:'Свежая туша на трёх камерах. 3 заряда. Полностью снимает голод Ползуна — до конца смены он сыт.',
  ...
  personal: {
    ...
    skills: [
      ...
      { id:'p_attention', name:'ВНИМАТЕЛЬНОСТЬ', icon:'👀',
        desc:'Глаза привыкли к темноте. NPC подсвечены ярче, камера с движением обведена рамкой.',
        maxLevel:2, cost:[1,1], effect:'attention',
        row:0, col:0, requires:[] },

      { id:'coffee', name:'ФОТОАППАРАТ', icon:'📷',
        desc:'Фотоаппарат — твоё оружие против Сталкера. +1 использование снимка за уровень.',
        maxLevel:2, cost:[1,1], effect:'snap_extra_uses',
        row:0, col:1, requires:[] },

      { id:'p_steady_hands', name:'АККУМУЛЯТОР', icon:'🔋',
        desc:'Резервный аккумулятор под полом. С ночи 13 — полная энергия. Без него оборудование умирает вместе с тобой.',
        maxLevel:1, cost:[1], effect:'has_battery',
        row:0, col:2, requires:[] },

      // ═══ РЯД 1 — ОПЫТНЫЙ ═══════════════════════════════════════════════
      { id:'focus', name:'КОНЦЕНТРАЦИЯ', icon:'🧠',
        desc:'Ты знаешь куда целиться. +5/15/25% к шансу что Сталкер отреагирует на снимок.',
        maxLevel:3, cost:[1,1,1], effect:'snap_bonus',
        row:1, col:0, requires:['p_attention'] },

      { id:'endurance', name:'ВЫНОСЛИВОСТЬ', icon:'🏃',
        desc:'Ужас давит, но ты держишься. Расход рассудка −7% за уровень, максимум −21%.',
        maxLevel:3, cost:[1,1,1], effect:'energy_save',
        row:1, col:1, requires:['coffee'] },

      { id:'quick_hands', name:'УМЕЛЫЙ', icon:'⚡',
        desc:'Руки уже знают что делать. Камеры переключаются без рывков и статики.',
      ...
      { id:'calm_nerves', name:'СТАЛЬНЫЕ НЕРВЫ', icon:'💪',
        desc:'+15% стартовый рассудок. После близкой смерти — 3 секунды тишины в голове.',
        maxLevel:1, cost:[1], effect:'scare_bonus',
        row:2, col:0, requires:['focus'] },

      { id:'p_deep_breath', name:'ГЛУБОКИЙ ВДОХ', icon:'💨',
        desc:'Закрыть глаза. Выдохнуть. Вернуть 25% рассудка. Активная способность, 2 раза за смену.',
        maxLevel:1, cost:[1], effect:'deep_breath',
        row:2, col:1, requires:['endurance'] },

      { id:'situational', name:'ТАКТИЧЕСКОЕ ЧУТЬЁ', icon:'👁',
        desc:'Тень видна с периметра — не только у двери. Видишь куда она уходит. Знание её пути — половина спасения.',
      ...
      { id:'night_vision', name:'НОЧНОЕ ЗРЕНИЕ', icon:'🌙',
        desc:'Глаза адаптировались к темноте. Камеры светлее на 25%. Чёрные углы стали серыми.',
        maxLevel:1, cost:[1], effect:'night_vision',
        row:3, col:0, requires:['calm_nerves'] },

      { id:'p_pattern_memory', name:'АНАЛИЗ ДВИЖЕНИЙ', icon:'📊',
        desc:'Сталкер ходит по одним маршрутам. На карте виден его след за последние 3 хода.',
        maxLevel:1, cost:[1], effect:'pattern_memory',
        row:3, col:1, requires:['p_deep_breath'] },

      { id:'premonition', name:'ШЕСТОЕ ЧУВСТВО', icon:'🔮',
        desc:'Что-то не так — ты чувствуешь это за ход до прихода NPC. Достаточно чтобы закрыть дверь.',
      ...
      { id:'p_iron_will', name:'ЖЕЛЕЗНАЯ ВОЛЯ', icon:'🏋',
        desc:'Иммунитет к панике. Фон не мигает. Рассудок не рушится при низком уровне. Soul drain медленнее на 50%.',
        maxLevel:1, cost:[1], effect:'iron_will',
        row:4, col:0, requires:['night_vision'] },

      { id:'p_predict', name:'ПРЕДСКАЗАНИЕ', icon:'🎱',
        desc:'На карте — стрелка следующего хода каждого NPC. Ты видишь на шаг вперёд. Этого хватает.',
        maxLevel:1, cost:[1], effect:'predict',
        row:4, col:1, requires:['p_pattern_memory'] },

      { id:'p_fast_switch', name:'РЕФЛЕКСЫ', icon:'🔄',
        desc:'1 раз за смену: прожектор включается сам когда тень идёт к офису. Тело реагирует раньше разума.',
      ...
      { id:'adrenaline', name:'АДРЕНАЛИН', icon:'💉',
        desc:'При <25% рассудка — все способности перезаряжаются мгновенно. +50% скорости отгона до конца смены. Страх превращается в топливо.',
        maxLevel:1, cost:[1], effect:'adrenaline',
        row:5, col:0, requires:['p_iron_will'] },

      { id:'p_meditation', name:'МЕДИТАЦИЯ', icon:'🧘',
        desc:'Пустота внутри. Все NPC замедлены на 50% на 20 сек. Кулдаун 60 сек. 2 раза за смену.',
        maxLevel:1, cost:[1], effect:'meditation',
        row:5, col:1, requires:['p_predict'] },

      { id:'survivor', name:'ВЫЖИВШИЙ', icon:'🛡',
        desc:'Один раз за смену: смертельный удар не убивает. Остаёшься с 1% рассудка. Полная неуязвимость 3 секунды.',
      ...
      { id:'p_time_stop', name:'ОСТАНОВКА ВРЕМЕНИ', icon:'⏸',
        desc:'Всё замирает. NPC стоят 30 секунд. Энергия не тратится. Любая способность — мгновенно. 1 раз за смену.',
        maxLevel:1, cost:[1], effect:'time_stop',
        row:6, col:0, requires:['adrenaline','p_meditation'] },

      { id:'p_foresight', name:'ЯСНОВИДЕНИЕ', icon:'🔭',
        desc:'Видишь ходы всех NPC на 3 шага вперёд. Включается каждую смену. Ты уже знаешь чем это закончится.',
        maxLevel:1, cost:[1], effect:'foresight',
        row:6, col:1, requires:['p_meditation','survivor'] },

      { id:'p_omniscience', name:'ВСЕВЕДЕНИЕ', icon:'👁‍🗨',
        desc:'Все NPC видны везде. Всегда. Без оборудования. Даже невидимые. Ты больше не охранник — ты что-то другое.',
  ...
  arms: {
    ...
    skills: [
      ...
      { id:'arms_unlock', name:'ОРУЖЕЙНИК', icon:'☠',
        desc:'Доступ к арсеналу. Военный платит вдвое меньше. Охранник — только после полной прокачки ОХОТНИКА. Учёному недоступно.',
        maxLevel:1, cost:[1], effect:'arms_unlock',
        row:0, col:1, requires:[] },

      // ── КОЛОНКА 1: ПРОКАЧКА ДРОБОВИКА (4 узла) ──
      { id:'arms_shotgun_ammo', name:'БОЕЗАПАС', icon:'🟠',
        desc:'Патроны найдены в подсобке. +1 заряд ружья за смену, итого 2.',
        maxLevel:1, cost:[1], effect:'arms_shotgun_ammo',
        row:1, col:0, requires:['arms_unlock'] },

      { id:'arms_shotgun_speed', name:'СКОРОСТРЕЛ', icon:'⏱',
        desc:'Перезарядка быстрее. Кулдаун снижен. Стреляй чаще — они не ждут.',
      ...
      { id:'arms_shotgun_buckshot', name:'КАРТЕЧЬ', icon:'💥',
        desc:'Картечь вместо дроби. NPC отброшен на 5 зон. Больше расстояния — больше времени.',
      ...
      { id:'arms_shotgun_pro', name:'ПРОФ. ДРОБОВИК', icon:'🔫',
        desc:'3 заряда, +50% к отбросу, стрельба прямо у двери. Это уже не обрез — это другой разговор.',
      ...
      { id:'arms_tripwire', name:'РАСТЯЖКА', icon:'⚡',
        desc:'Проволока с детонатором на камере. NPC проходит — стан 12 сек. 1 заряд за смену.',
        maxLevel:1, cost:[1], effect:'arms_tripwire',
        row:1, col:1, requires:['arms_unlock'] },

      { id:'arms_mine', name:'ПРОТИВОПЕХ. МИНА', icon:'💣',
        desc:'Противопехотная мина на камере. NPC проходит — выведен из боя на всю смену. 1 заряд.',
        maxLevel:1, cost:[1], effect:'arms_mine',
        row:2, col:1, requires:['arms_tripwire'] },

      { id:'arms_flare', name:'РАКЕТНИЦА', icon:'🚨',
        desc:'Сигнальная ракета. Все NPC вокруг бегут прочь 20 секунд. 1 заряд. Огонь — универсальный язык.',
      ...
      { id:'arms_radio', name:'РАДИОСВЯЗЬ', icon:'📡',
        desc:'Тактическая рация. Все NPC всегда видны на карте. Знание — лучшая броня.',
        maxLevel:1, cost:[1], effect:'arms_radio',
        row:1, col:2, requires:['arms_unlock'] },

      { id:'arms_thermal', name:'ТЕПЛОВИЗОР', icon:'🌡',
        desc:'Тепловизор. Видишь силуэты NPC в полной темноте — без камер, без света.',
        maxLevel:1, cost:[1], effect:'arms_thermal',
        row:2, col:2, requires:['arms_radio'] },

      { id:'arms_reflexes', name:'РЕФЛЕКСЫ', icon:'⚡',
        desc:'Боевые рефлексы. Переключение камер быстрее на 1 сек. Фонарь — мгновенно.',
        maxLevel:1, cost:[1], effect:'arms_reflexes',
        row:3, col:2, requires:['arms_thermal'] },

      // ── ФИНАЛ: ТЕРМОЯДЕРКА ──
      { id:'arms_thermo', name:'ТЕРМОЯДЕРКА', icon:'☢',
        desc:'Термоядерный заряд под объектом. Резонанс, NPC и ты — уничтожены. Кнопка «ВЗОРВАТЬ» появляется в офисе. Некоторые проблемы решаются только так.',
  ...
  lab: {
    ...
    skills: [
      ...
      { id:'lab_unlock', name:'ЛАБОРАНТ', icon:'🔬',
        desc:'Доступ к лаборатории. Учёному открыто изначально. Охранник — только после прокачки ОХОТНИКА. Военному недоступно.',
